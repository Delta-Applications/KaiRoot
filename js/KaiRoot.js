...
var rootingScript = [
  ...
  'start adbd'
   ...
   if (Wallace.getSystemProperty("persist.net.tcp.buffersize.lte") == "524288,1048576,2097152,524288,1048576,2097152"){document.querySelector("#buffer-text").innerText = "Boosted! ✨"}
   function utilityIncreaseBufferSize(){
    if (Wallace.getSystemProperty("persist.net.tcp.buffersize.lte")) return;
          ...
          if (enableSim2Actions) {document.querySelector("#multisim-title").innerText = "Disable SIM 2"}else{document.querySelector("#multisim-title").innerText = "Enable SIM 2"}
        function utilityMultiSim(){
          if (confirm("Apply SIM Patch and Reboot? Toggling to Single might make your battery last longer.")){
        ...
        function dangerChangeWifiMac(){
         ...
         if (Wallace.getSystemProperty('sys.usb.config') === 'diag,serial_smd,rmnet_qti_bam,adb') {
          ...
          function developerMenu(){
            if (window.confirm(translate("devm_ask"))){
              return new MozActivity({name: "configure",
                                      data: {
                                       target: "device",
                                       section: "developer"
                                            }})
            ...
            if(window.confirm(translate("devm_ask2")))
